...
export default function CrudPage({ title, subtitle, endpoint, fields, columns, initial = {}, searchKeys = ['name'] }) {
  ...
  function newRecord(){ setEditing(null); setForm(initial); setOpen(true) }
  function editRecord(row){ setEditing(row); const next={}; fields.forEach(f=>next[f.name]=row[f.readKey || f.name] ?? ''); setForm(next); setOpen(true) }
  ...
  async function remove(row){ if(!confirm(`Delete ${row.name || row.code || 'this record'}?`))return; try{await api(`${endpoint}${row.id}/`,{method:'DELETE'}); await load()}catch(e){setError(e.message)} }
  return <>
    <PageHeader title={title} subtitle={subtitle} actions={<Button onClick={newRecord}>+ Add New</Button>}/>
    <Card><div className="toolbar"><input className="search" placeholder="Search…" value={q} onChange={e=>setQ(e.target.value)}/><span>{filtered.length} records</span></div><ErrorBox error={error}/>{loading?<div className="loading">Loading…</div>:filtered.length===0?<Empty/>:<div className="table-wrap"><table><thead><tr>{columns.map(c=><th key={c.key}>{c.label}</th>)}<th>Actions</th></tr></thead><tbody>{filtered.map(row=><tr key={row.id}>{columns.map(c=><td key={c.key}>{c.render?c.render(row):String(row[c.key]??'')}</td>)}<td className="actions"><button onClick={()=>editRecord(row)}>Edit</button><button className="danger-link" onClick={()=>remove(row)}>Delete</button></td></tr>)}</tbody></table></div>}</Card>
