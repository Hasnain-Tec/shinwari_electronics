...
def _get_attr_decimal(obj, names):
    for name in names:
        try:
            if hasattr(obj, name):
...
def _get_direct_amount_paid(sale):
    return _get_attr_decimal(sale, [
        'amount_paid',
        'paid_amount',
        'paid',
...
def _get_payment_records(sale):
    ...
    for name in relation_names:
        try:
            relation = getattr(sale, name, None)

            if relation is None:
                continue

            records = relation.all() if hasattr(relation, 'all') else relation
...
def _get_total_paid(sale):
    """
    Calculate Amount Paid from the actual payment history.

    Payment history is the source of truth.
    The sale.amount_paid field is only used as a fallback
    when no payment history relation is available.
    """

    records = _get_payment_records(sale)

    if records:
        total = Decimal('0')

        for payment in records:
            total += _get_attr_decimal(
                payment,
                ['amount', 'paid_amount', 'payment_amount']
            )

        return total

    return _get_direct_amount_paid(sale)


def _get_payment_description(sale):
    """
    KEEPING THE AMOUNT PAID HISTORY DISPLAY LOGIC UNCHANGED.

    This function is intentionally not changed to calculate
    or replace the payment history display.
    """
    ...
    if len(amounts) > 1:
        parts = [f'{amount:,.2f}' for amount in amounts]
        return ' + '.join(parts) + f' = {sum(amounts, Decimal("0")):,.2f}'

    if len(amounts) == 1:
        return f'{amounts[0]:,.2f}'

    total_paid = _get_total_paid(sale)

    return f'{total_paid:,.2f}' if total_paid else '0.00'


def _get_remaining_balance(sale, total_paid):
    """
    Remaining Balance is always calculated from:

        Invoice Total - Actual Payment History Total

    We intentionally do NOT use sale.remaining_balance,
    sale.balance_due, etc. because those values may be stale.
    """

    invoice_total = _decimal(getattr(sale, 'total', 0))
...
def _draw_info_boxes(c, sale, y_top):
    ...
    if sale.salesperson:
        salesperson = (
            sale.salesperson.get_full_name().strip()
            or sale.salesperson.username
        )
    ...
    # Amount Paid is calculated from actual payment history.
    total_paid = _get_total_paid(sale)

    # Total Balance is calculated from sale total minus payment history.
    total_balance = _get_remaining_balance(
        sale,
        total_paid
    )
...
def _draw_last_page_footer(c, sale, y_top, table_bottom, setting):
    ...
    # IMPORTANT:
    # Amount Paid is taken from the actual payment history.
    total_paid = _get_total_paid(sale)

    # Remaining Balance is Sale Total - actual payments.
    remaining_balance = _get_remaining_balance(
        sale,
        total_paid
    )

    # KEEP Amount Paid History exactly as before.
    description = _get_payment_description(sale)

    rows = [
        ('Discount', sale.discount_total),
         ('Total Amount', sale.total),
        ('Amount Paid', total_paid),
        ('Amount Paid History', description),
    ...
    for idx, (label, value) in enumerate(rows):
        ...
        if label == 'Amount Paid History':
            # DO NOT CHANGE THE HISTORY DISPLAY.
            c.setFont(
                'Helvetica',
                7.5
            )
    ...
    for page_idx, chunk in enumerate(chunks, 1):
        ...
        if page_idx == 1:
            ...
            c.drawString(
                7*mm,
                y-2*mm,
                f'Invoice {sale.invoice_no} - Continued'
            )
        ...
        table_bottom, _ = _draw_items_table(
            c,
            numbered,
            y,
            max_rows,
            continued=page_idx > 1
        )

        if page_idx == total_pages:

            _draw_last_page_footer(
                c,
                sale,
                table_bottom-1*mm,
                table_bottom,
...
def build_receipt_pdf(payment):
    ...
    rows = [
        ...
        ['Type', payment.get_party_type_display()],
        ['Party', party],
        ['Amount', _money(payment.amount, setting)],
